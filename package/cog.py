...
class Achievement(commands.GroupCog):
    ...
    async def claim(
        ...
        rewards = await AchievementReward.filter(achievement_id=achievement.id).values_list("ball_id", flat=True)

for reward_ball_id in rewards:
    await BallInstance.create(
        player=player,
        ball_id=reward_ball_id,  
        special=None,
        health_bonus=random.randint(-settings.max_health_bonus, settings.max_health_bonus),  # ✅ correct cap
